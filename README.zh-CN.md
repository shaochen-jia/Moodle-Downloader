...
自动把(Monash)Moodle 上发布的每一份讲义、习题和作业要求归入周文件夹,
整个学期自行运行。晚发布的答案会在下一次同步时自动补上,任何文件都不会
重复下载。
...
- **讲座字幕**:录播视频本身通常无法下载,但字幕可以。Panopto(需额外
  单点登录一次)和 YouTube 的录播会被转成可读文稿,保存在
  `Week NN/Transcripts/` 里。
