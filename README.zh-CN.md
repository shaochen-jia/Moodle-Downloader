...
- **AI 总结(可选)**:在设置界面选一个服务商并填入 API key,每份文稿会
  额外生成一段要点总结。支持 Gemini(有免费额度)、Claude、OpenAI、
  DeepSeek、Kimi、智谱 GLM、通义千问,以及本地 Ollama。不开启的话,课程
  内容不会发送到任何地方,也不会出现这部分。

## 设置项

课程、下载目录、同步间隔和 AI 服务商都可以在程序窗口里设置。其余选项在
`config.yaml` 里,该文件按用户保存在 `%LOCALAPPDATA%\moodle-downloader\`,
**更换或移动程序都不会丢失设置**:
